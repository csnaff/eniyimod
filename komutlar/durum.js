...
exports.run = async (client, message, args) => {
    ...
    if(KaraListe) return  message.react("<a:Heaven_Hayr:876940416874868768>");

    if(client.config.SadeceTaglı) {
    if (!message.member.roles.cache.has(client.config.teyitci))
    if(message.author.id !== client.config.sahip) 
    if(!message.member.roles.cache.has(client.config.owner)) return  message.react("<a:Heaven_Hayr:876940416874868768>");
  ...
  const embed = new Discord.MessageEmbed().setColor("RANDOM").setThumbnail(kullanici.avatarURL({dynamic: true, size: 2048}))
